...
public class LiquidFloodFill extends Module {

    private final DoubleSetting range = register(new DoubleSetting(
            "Range", "Search radius for liquid blocks", 4.0, 1.0, 8.0));
    private final BoolSetting lava = register(new BoolSetting(
            "Lava", "Fill lava source blocks", true));
    private final BoolSetting water = register(new BoolSetting(
            "Water", "Fill water source blocks", false));

    private final TimerUtil timer = new TimerUtil();

    public LiquidFloodFill() {
        super("LiquidFloodFill", "Fills water/lava surfaces with solid blocks", Category.WORLD);
    }
    ...
    public void onTick(EventTick event) {
        ...
        for (BlockPos pos : BlockPos.iterate(center.add(-r, -2, -r), center.add(r, 2, r))) {
            ...
            boolean isLava  = lava.isEnabled()  && state.isOf(Blocks.LAVA);
            boolean isWater = water.isEnabled() && state.isOf(Blocks.WATER);
