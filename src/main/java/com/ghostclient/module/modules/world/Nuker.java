 ...
 *
 * Modes:
 *   All   – break every non-air block in range
 *   Flat  – only break blocks at or below player's foot level
 *   ID    – only break a specific block type (set blockID)
 *
 * Shapes:
 *   Sphere – only blocks within the radius (Euclidean distance)
 *   Cube   – all blocks in the bounding cube
 */
